...
class Statistics:
    ...
    def median(data: List[float]) -> float:
        n = len(data)
        if n == 0: raise ValueError("Empty data")
        s_data = sorted(data)
        mid = n // 2
        if n % 2 == 1:
            return s_data[mid]
    ...
    def variance(data: List[float], sample: bool = True) -> float:
        n = len(data)
        if n < 2: raise ValueError("Needs at least 2 points")
    ...
    def std_dev(data: List[float], sample: bool = True) -> float:
        # Zero-wrapper: Using our custom Newton-Raphson sqrt
        return sqrt_custom(Statistics.variance(data, sample))

    @staticmethod
    def z_score(x: float, data: Optional[List[float]] = None, 
                mu: Optional[float] = None, sigma: Optional[float] = None) -> float:
        """Professional Z-Score: Supports both raw data or pre-calculated params."""
        if data is not None:
            mu = Statistics.mean(data)
    ...
    def iqr(data: List[float]) -> float:
        n = len(data)
        if n < 4: raise ValueError("Needs at least 4 points")
        s_data = sorted(data)
        mid = n // 2
        low = s_data[:mid]
    ...
    def correlation(x: List[float], y: List[float]) -> float:
        ...
        den = sqrt_custom(sum((xi - mu_x)**2 for xi in x) * sum((yi - mu_y)**2 for yi in y))
